...
class G1ML3DDataset(Dataset):
    """
    Dataset for `data/pkls/g1ml3d_v1.pkl` generated by `data/preprocess_g1ml3d.py`.

    Each motion_item contains:
      - dof_pos: (T, 36)
      - audio_feat: (T, 512) - CLIP text features (stored in audio_feat field for compatibility)
    ...
    def __init__(self, pkl_path, offset_frame, past_frame, future_frame, dtype=np.float32, limited_num=-1):
        ...
        self.dof_list = []
        self.text_feat_list = []  # CLIP text features
        frame_nums = []
        ...
        for motion_item in tqdm(motions):
            ...
            text_feat = motion_item['audio_feat'].astype(dtype)  # (T, 512) - CLIP text features
            frame_num = dof.shape[0]
            if frame_num < window_size:
                continue
            if text_feat.shape[0] != frame_num:
                raise RuntimeError(
                    f"Frame mismatch in {motion_item.get('filepath', '<unknown>')}: dof={frame_num}, text_feat={text_feat.shape[0]}"
                )

            frame_nums.append(frame_num)
            self.dof_list.append(dof)
            self.text_feat_list.append(text_feat)
        ...
        self.audio_dim = int(self.text_feat_list[0].shape[-1])  # Use audio_dim name for compatibility
        self.joint_num = 1
        ...
        print(
            'G1ML3DDataset loaded, trained with %d clips, %d frames, %d mins in total'
            % (len(frame_nums), sum(frame_nums), sum(frame_nums) / 20 / 60)  # Assuming 20 fps for G1ML3D
        )
    ...
    def __getitem__(self, idx):
        ...
        text_feat = self.text_feat_list[motion_idx][frame_indices].copy()  # (window, text_feat_dim)

        past_dof = dof[: self.reference_frame_idx]
        future_dof = dof[self.reference_frame_idx :]

        future_text_feat = text_feat[self.reference_frame_idx :]  # align with future frames
        ...
        audio_feat = torch.from_numpy(future_text_feat.T)  # (text_feat_dim, future) - use audio_feat name for compatibility

        return {
            'data': future_motion,
            'conditions': {
                'past_motion': past_motion,
                'audio_feat': audio_feat,  # Actually text features, but use audio_feat name for model compatibility
